...
void OpenGLControl::OnTimer(UINT_PTR nIDEvent)
{
	...
	switch (nIDEvent)
	{
		case 1:
		{
			if (stuff == DUONG_TRON) 
			{
				//tang bien dieu khien dem
				//drawStuff.m_count = drawStuff.m_count + m_timer;
				drawStuff.m_count = dem;
			}
			else if (stuff == XOAN_REN) 
			{
				//tang bien dieu khien dem
				drawStuff.m_time = dem;
			}
			else if (stuff == TRAI_TIM) 
			{
				//tang bien dieu khien dem

				drawStuff.m_time1 = dem;
			}
			else if (stuff == COMPOUND) 
			{
				//tang bien dieu khien dem
				drawStuff.m_q1 = q1;
				drawStuff.m_q2 = q2;
...
void OpenGLControl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{
		_fZoom -= (float)2.0f * diffY;
	}

	// Middle mouse button
	else if (nFlags & MK_MBUTTON)
	{
		_fPosX += (float)0.1f * diffX;
		_fPosY -= (float)0.1f * diffY;
	}
